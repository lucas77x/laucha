...
//
// laucha patch: not part of upstream golang.design/x/hotkey v0.6.1.
...
//
// laucha patch: only events matching the registered combination are reported.
// grabHotkey also selects KeyPressMask on the root window, so unrelated keys
// reach this loop as well -- for instance those the server replays when another
// client (a menu with its own keyboard grab) lets go of the keyboard. Upstream
// fired the callback for every one of them, which made the launcher pop up on
// foreign shortcuts. Releases only match while the modifiers are still held,
// which is enough for the keyup channel this package exposes.
